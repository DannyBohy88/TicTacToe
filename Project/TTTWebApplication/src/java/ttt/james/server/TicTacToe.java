...
public class TicTacToe {
    ...
    // Panels
        ...
        // Overall Leaderboard
        public static JPanel lbw_panel;
    // Labels
        // Main Window
        ...
        // Show Games Window
        public static JLabel sgw_titleLbl;
        public static JLabel sgw_joinGameLbl;
        // Leaderboard Panel
        public static JLabel lbw_titleLbl;
    // Buttons
        //Main Window
        ...
        public static JButton gw_showMyHistoryBtn;
        public static JButton gw_showLeaderboardBtn;
        public static JButton gw_hostgameBtn;
        public static JButton gw_logoutBtn;
        // Show Games Window
        public static JButton sgw_joinGameBtn;
        // Leaderboard Window
        public static JButton lbw_backBtn;
    //Textfields
        // Main Window
        ...
        // Show Games Window
        public static JTextField sgw_joinGameTF;
    // JTables
        //Open Games
        public static JTable sgw_openGameTBL;
        // Leaderboard Window
        public static JTable lbw_leaderboardTBL;
    ...
    public static void initWindows(){
       ...
       windows.get(2).setTitle("Lobby");
        ...
        gw_showLeaderboardBtn = new JButton("Show Leaderboard");
        gw_showMyHistoryBtn = new JButton("Show History");
        gw_findgameBtn = new JButton("Find Game");
        gw_hostgameBtn = new JButton("Host Game");
        gw_logoutBtn = new JButton("Logout");
        gw_showLeaderboardBtn.addActionListener(al);
        gw_showMyHistoryBtn.addActionListener(al);
        ...
        gw_panel.add(gw_showLeaderboardBtn);
        gw_panel.add(gw_showMyHistoryBtn);
        gw_panel.add(gw_logoutBtn);
        windows.get(2).add(gw_panel);
       windows.add(new JFrame());
       windows.get(3).setTitle("OpenGames");
            sgw_panel = new JPanel();
            sgw_titleLbl = new JLabel("Open Games");
            sgw_joinGameLbl = new JLabel("Join Game");
            Object data [][] = new String[50][50];
            String colNames [] = {"ID","p1","date"};
            sgw_openGameTBL = new JTable(data,colNames);
            sgw_joinGameTF = new JTextField();
            sgw_panel.setLayout(new GridLayout(5,1));
            sgw_panel.add(sgw_titleLbl);
            sgw_panel.add(sgw_openGameTBL);
            sgw_panel.add(sgw_joinGameLbl);
            sgw_panel.add(sgw_joinGameTF);
       windows.get(3).add(sgw_panel);
       windows.add(new JFrame());
       windows.get(4).setTitle("Overall Leaderboard");
            lbw_panel = new JPanel();
            lbw_titleLbl = new JLabel("Overall Leaderboard");
            data = new String [50][50];
            String columns [] = {"Player","Wins","Losses","Draws"};
            lbw_leaderboardTBL = new JTable(data,columns);
            lbw_leaderboardTBL.setBounds(0, 0, 500, 500);
            lbw_backBtn = new JButton("Back");
            lbw_panel.setLayout(new GridLayout(3,1));
            lbw_panel.add(lbw_titleLbl);
            lbw_panel.add(lbw_leaderboardTBL);
            lbw_panel.add(lbw_backBtn);
       windows.get(4).add(lbw_panel);
    ...
    public static void clearWindowText(){
        ...
        rw_forenameTF.setText("");
        rw_surnameTF.setText("");
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(source.equals(rw_backBtn)){
            ...
            System.out.println(rw_usernameTF.getText());
            System.out.println(rw_passwordTF.getText());
            System.out.println(rw_forenameTF.getText());
            System.out.println(rw_surnameTF.getText());
            System.out.print(webService.register(rw_usernameTF.getText(),rw_passwordTF.getText(),rw_forenameTF.getText(),rw_surnameTF.getText()));
            showWindow(0);
        }
        // Game Window
        if(source.equals(gw_findgameBtn)){
            ...
            if(!res.matches("ERROR-NOGAMES")){
                String temp [] = res.split(",");
                ArrayList<Game> g = new ArrayList<Game>();
                g.add(new Game());
                int size = temp[2].length()-1;
                g.get(0).setGameID(Integer.parseInt(temp[0]));
                g.get(0).setPlayer1(temp[1]);
                g.get(0).setStarted(temp[2]);
                g.add(new Game());
                g.get(1).setGameID(Integer.parseInt(temp[2].subSequence(22,temp[2].length()).toString()));
                g.get(1).setPlayer1(temp[3]);
                g.get(1).setStarted(temp[4].subSequence(0,22).toString());
                for(int i = 4 ; i < (temp.length-2) ; i += 2){
                   Game t = new Game();
                   int value = Integer.parseInt(temp[i].subSequence(22,temp[i].length()).toString());
                   t.setGameID(value);
                   t.setPlayer1(temp[i+1]);
                ...
                int count =0;
               for( Game g2 :  g){
                   sgw_openGameTBL.setValueAt(g2.getID() + "" ,row,0);
                   sgw_openGameTBL.setValueAt(g2.getPlayer1(),row,1);
                   sgw_openGameTBL.setValueAt(g2.getStarted() + "" ,row,2);
                   row++;
               }
            }
            showWindow(3);
        }else if(source.equals(gw_hostgameBtn)){
               System.out.print(webService.newGame(currentID));
        
        }else if(source.equals(gw_logoutBtn)){
            currentUsername = "";
            showWindow(0);
        }else if(source.equals(gw_showLeaderboardBtn)){
            showWindow(4);
        }
        // Join Game Window
        if(source.equals(sgw_joinGameBtn)){
            int gameToJoin = Integer.parseInt(sgw_joinGameTF.getText());
            webService.joinGame(currentID,gameToJoin);
